...
int stream_load(double times[4][NTIMES], int myrank) {
	char placebo[3];
	
	char buffer[64];
	snprintf(buffer, 64, "./data/k_rank%d.dat\0", myrank);
...
void stream_write(int k, double time_log[4][NTIMES], int myrank) {
	...
	snprintf(buffer, 64, "./data/time_rank%d.dat\0", myrank);
	fprintf(stderr, buffer);
...
main()
	...
	if(myrank==0){
		printf("%d MPI ranks run STREAM_ARRAY_SIZE=%d of %zu bytes for %d Iterations\n", 
				numranks, STREAM_ARRAY_SIZE, sizeof(STREAM_TYPE), NTIMES);
	}
...
#pragma omp parallel for
    ...
    for (k=0; k<NTIMES; k++)
	{
		if(k%10==0 && myrank==0) fprintf(stderr, "%d output %d \n", k, NTIMES);

		if(backup == 1) {
			stream_write(k, times, myrank);
			...
			if (fp)  { // file exists, load times
				stream_load(times, myrank);
				k = k + qq;
				backup = 2; // Ensure this is executed only once
				printf("Rank %d RESUMING before MPI_Barrier()...!!\n", myrank);
				MPI_Barrier(MPI_COMM_WORLD); //wait for all MPI ranks to finish loading
				printf("Rank %d RESUMING after MPI_Barrier()...!!\n", myrank);
			}
		}
